...
import {
  getServiceProviderEvaluations,
  createServiceProviderEvaluation,
  updateServiceProviderEvaluation,
  deleteServiceProviderEvaluation,
} from "../../../utils/serviceProviderApi";
import ConfirmationDialog from "../../../components/ConfirmationDialog.jsx";

const ServiceProviderEvaluation = ({ serviceProviderId, assignedCriteria = [] }) => {
  ...
  const fetchEvaluations = async () => {
    try {
      setLoading(true);
      const data = await getServiceProviderEvaluations(serviceProviderId);
      setEvaluations(data);
    } catch (error) {
  ...
  const handleSaveNew = async () => {
    ...
    try {
      await createServiceProviderEvaluation(serviceProviderId, newRow);
  ...
  const handleDoneEdit = async (id) => {
    ...
    try {
      await updateServiceProviderEvaluation(id, {
        quarter: row.quarter,
  ...
  const handleDeleteRow = async () => {
    ...
    try {
      await deleteServiceProviderEvaluation(evaluationToDelete.id);
  ...
  return (
    <div className="bg-white p-6 rounded shadow mt-6">
      <div className="flex justify-between items-center mb-6">
        <h5 className="text-lg font-semibold">Service Provider Evaluation</h5>
        <button
...
export default ServiceProviderEvaluation;
