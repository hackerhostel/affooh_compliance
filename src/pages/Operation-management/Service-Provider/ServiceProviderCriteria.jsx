...
const ServiceProviderCriteria = () => {
  ...
  const fetchCriteria = async () => {
    try {
      setLoading(true);
      const data = await getServiceProviderCriteria();
  ...
  const handleSaveNew = async () => {
    ...
    try {
      await createServiceProviderCriteria({ ...newRow, organizationID: 1 });
  ...
  const handleDoneEdit = async (id) => {
    ...
    try {
      await updateServiceProviderCriteria(id, { name: row.name });
  ...
  const handleDeleteRow = async () => {
    ...
    try {
      await deleteServiceProviderCriteria(criteriaToDelete.id);
