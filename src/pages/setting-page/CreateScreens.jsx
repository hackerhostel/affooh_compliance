...
import { XMarkIcon, TrashIcon, PlusCircleIcon } from '@heroicons/react/24/outline';
...
const CreateNewScreen = ({ isOpen, onClose }) => {
    ...
    const handleSelectField = (name, value) => {
        const exists = selectedFields.find(f => f.id.toString() === value.toString());
        if (!exists) {
            const option = customFieldOptions.find(opt => opt.value.toString() === value.toString());
                                ...
                                {showGeneralInput && (
                                    <div className="flex items-center space-x-2 justify-between">
                                        <FormInput
                                            ...
                                            onChange={({ target: { value } }) => setGeneralInputValue(value)}
                                        />
                                        <button
                                            type="button"
                                            ...
                                            onClick={() => {
                                                if (generalInputValue.trim()) {
                                                    setGeneralList(prev => [...prev, generalInputValue.trim()]);
                                        ...
                                        <div key={index} className="flex items-center gap-2">
                                            <div className="flex items-center bg-primary-pink text-white px-4 py-2 rounded-full text-sm font-medium">
                                                ...
                                                <XMarkIcon
                                                    ...
                                                    onClick={() =>
                                                        setGeneralList(prev => prev.filter((_, i) => i !== index))
                                                    }
                                                />
                                            </div>
                                        </div>
                                        ...
                                        <div key={field.id} className="flex items-center gap-2">
                                            ...
                                            <div className="flex items-center space-x-2 text-sm">
                                                <input
                                                    ...
                                                    onChange={(e) =>
                                                        updateFieldStatus(field.id, e.target.checked ? 'required' : 'optional')
                                                    }
