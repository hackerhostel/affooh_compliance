...
const CommunicationRegisterHistory = () => {
  ...
  const { data: contextData } = useFetchOrganizationalContext(projectId, 'Communication');
  const { data: revisionHistory } = useFetchRevisionHistory(projectId, 'Communication');
  const { data: approvals } = useFetchApprovals(projectId, 'Communication');
  ...
  const revisionRows = useMemo(() => {
    return (revisionHistory || []).map((item) => ({
      name: contextData?.documentType || "Communication",
