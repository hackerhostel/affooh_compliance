...
const StakeholderHistory = () => {
  const { addToast } = useToasts();
  const selectedProject = useSelector(selectSelectedProject);
  const projectUserList = useSelector(selectProjectUserList);
  const projectId = selectedProject?.id;

  const { data: contextData } = useFetchOrganizationalContext(projectId, 'Stakeholder');
  const { data: revisionHistory } = useFetchRevisionHistory(projectId, 'Stakeholder');
  const { data: approvals } = useFetchApprovals(projectId, 'Stakeholder');
  ...
  const revisionRows = useMemo(() => {
    return (revisionHistory || []).map((item) => ({
      name: contextData?.documentType || "Stakeholder",
...
export default StakeholderHistory;
