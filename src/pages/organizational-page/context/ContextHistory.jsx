...
const ContextHistory = () => {
  ...
  const { data: contextData } = useFetchOrganizationalContext(projectId, 'Context');
  const { data: revisionHistory } = useFetchRevisionHistory(projectId, 'Context');
  const { data: approvals } = useFetchApprovals(projectId, 'Context');
  ...
  const revisionRows = useMemo(() => {
    return (revisionHistory || []).map((item) => ({
      name: contextData?.documentType || "Context",
