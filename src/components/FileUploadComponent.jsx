...
const FileUploadComponent = React.forwardRef(({
  ...
  const handleFileUploads = async (taskID) => {
    if (attachments.length === 0) {      addToast("Please add at least one file", {
        appearance: "error",
        autoDismiss: true,
      });
    ...
    for (let i = 0; i < attachments.length; i++) {
      if (isPaused) {        addToast("Upload has been paused", {
          appearance: "info",
          autoDismiss: true,
        });
        return {
  ...
  const handleCancelUpload = () => {
    ...
    setIsUploading(false);
    setUploadProgress(0);
    setCurrentFileIndex(0);    addToast("Upload has been cancelled", {
      appearance: "warning",
      autoDismiss: true,
    });
  };
  ...
  return (    <div className="border-2 border-dashed border-gray-300 rounded-lg p-4">
